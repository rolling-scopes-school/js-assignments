...
function extractNameFromTemplate(value) {
  const withoutHello = value.split(',')[1].trim();

  return withoutHello.slice(0, withoutHello.length - 1);
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map(sym => {
      const index = input.indexOf(sym);

      return index !== -1 ? output[index] : sym;
    })
    .join('');
}
...
function getCardId(value) {
  ...
  const typeIndex = types.indexOf(cardType);
