...
function extractNameFromTemplate(value) {
    return value.slice(value.search(' ') + 1, value.length - 1);
}
...
function getRectangleString(width, height) {
	...
	var hor = '─';
	var space = ' ';
    return ('┌' + hor.repeat(width - 2) + '┐' + '\n') + 
    	   (vert + space.repeat(width - 2) + vert + '\n').repeat(height - 2) + 
    	   ('└' + hor.repeat(width - 2) + '┘' + '\n');
}
...
function encodeToRot13(str) {
  var plain = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  var key = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  var index = x => plain.indexOf(x);
  var translate = x => index(x) > -1 ? key[index(x)] : x;
