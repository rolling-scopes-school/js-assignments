...
function timespanToHumanString(startDate, endDate) {
    ...
    if (year > 1.5) return -Math.round(-year) + ' years ago';
    else 
        if (day > 345 && year <= 1.5) return 'a year ago';
    else 
        if (day > 45 && day <= 345) {
        ...
        if (day > 25 && day <= 45) return 'a month ago'; 
    else 
        if (hour > 36 && day <= 25) return -Math.round(-day) + ' days ago';
    else 
        if (hour > 22 && hour <= 36) return 'a day ago';
    else 
        if (minute > 90 && hour <= 22) return -Math.round(-hour) + ' hours ago';
    else 
        if (minute > 45 && minute <= 90) return 'an hour ago';
    else 
        if (second > 90 && minute <= 45) return -Math.round(-minute) + ' minutes ago';
    else 
        if (second > 45 && second <= 90) return 'a minute ago';
...
function getCommonDirectoryPath(pathes) {
    return pathes.map(path => path.split('/')).reduce((common, folder) => { let match = true;
